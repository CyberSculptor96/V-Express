...
# 输入和输出路径
input_dir = '/shareddisk/yexin/huanghj/data/TalkVid-160h/videos'
audio_dir = "/shareddisk/yexin/shunian/252_hour_test_480p_49frames/original_audio"
output_dir = '/shareddisk/yexin/huanghj/data/TalkVid-160h/short_clip_aud_embeds'
...
if __name__ == "__main__":
    process = 20000
    curr = args.gpu_id
    video_files = sorted([f for f in os.listdir(input_dir) if f.endswith('.mp4')])[curr*process:(curr+1)*process]
    ...
    for video_path in tqdm(video_files):
        ...
        original_audio_path = os.path.join(audio_dir, video_path.replace('.mp4', '.m4a'))
        aud_embeds_path = os.path.join(output_dir, video_path.replace('.mp4', '.pt'))

        if os.path.exists(aud_embeds_path):
            # print(f'Skipping {video_path}, audio embedding already exists at {aud_embeds_path}.')
