...
def compute_fvd(clip_timestamp, videos1, videos2, i3d, device, queue):
    """
    并行计算 FVD 并存入队列
    """
    # 独立的 CUDA Stream，防止进程间阻塞
    stream = torch.cuda.Stream()
    with torch.cuda.stream(stream):
        # 获取视频 clip
        videos_clip1 = videos1[:, :, :clip_timestamp].to(device, non_blocking=True)
        videos_clip2 = videos2[:, :, :clip_timestamp].to(device, non_blocking=True)
    ...
    queue.put((clip_timestamp, fvd_score))  # 将结果存入队列

def calculate_fvd(videos1, videos2, device, method='styleganv', only_final=False, num_workers=8):
    """
    计算 FVD，支持 GPU 多进程并行计算
    """
    if method == 'styleganv':
    ...
    if only_final:
        ...
        mp.set_start_method('spawn', force=True)  # 适用于 CUDA 计算
        queue = mp.Queue()  # 进程间通信队列
        processes = []
        fvd_dict = {}

        clip_range = range(10, videos1.shape[-3] + 1)

        # 启动多个进程并行计算 FVD
        for clip_timestamp in tqdm(clip_range, desc="Computing FVD"):
            if len(processes) >= num_workers:
                processes[0].join()  # 等待最早启动的进程结束
                processes.pop(0)  # 移除已完成的进程

            p = mp.Process(target=compute_fvd, args=(clip_timestamp, videos1, videos2, i3d, device, queue))
            p.start()
            processes.append(p)
        ...
        while not queue.empty():
            clip_timestamp, fvd_score = queue.get()
            fvd_dict[clip_timestamp] = fvd_score

        # 按时间戳排序
        fvd_results = [fvd_dict[t] for t in sorted(fvd_dict.keys())]
...
def main():
    # 真实视频路径和生成视频路径
    real_video_path = "/wangbenyou/huanghj/data/hallo3-dataset/videos"
    # gen_video_path = "/wangbenyou/huanghj/workspace/hallo3/output/output_testset"
    gen_video_path = "/wangbenyou/huanghj/data/fastvideo-hallo3-dataset/media"
    ...
    NUMBER_OF_VIDEOS = min(nums_of_video_real, nums_of_video_gen, 100)
    ...
    # 计算 FVD 分数
    result = calculate_fvd(real_videos, generated_videos, device, method='videogpt', only_final=False)
    print("[fvd-videogpt ]", result["value"])

    # result = calculate_fvd(real_videos, generated_videos, device, method='styleganv', only_final=False)
